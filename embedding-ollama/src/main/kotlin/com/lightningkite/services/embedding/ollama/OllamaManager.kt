...
/**
 * Manages an Ollama server process and model availability for embedding use cases.
 *
 ...
 */
public class OllamaEmbeddingManager(
    ...
    public suspend fun pullModel(
        model: String,
        onProgress: (OllamaEmbeddingPullProgress) -> Unit = {},
    ) {
        logger.info { "Pulling model: $model" }

        httpClient.preparePost("$baseUrl/api/pull") {
            contentType(ContentType.Application.Json)
            setBody(OllamaEmbeddingPullRequest(name = model))
        }.execute { response ->
            if (!response.status.isSuccess()) {
            ...
            while (!channel.isClosedForRead) {
                ...
                try {
                    val progress = json.decodeFromString(OllamaEmbeddingPullProgress.serializer(), line)
                    onProgress(progress)
    ...
    public suspend fun ensureReady(
        ...
        onProgress: (OllamaEmbeddingPullProgress) -> Unit = { progress ->
...
// Public DTOs for the manager — distinct from the private wire DTOs used by
// OllamaEmbeddingService to keep the public API stable.

@Serializable
public data class OllamaEmbeddingTagsResponse(
    val models: List<OllamaEmbeddingTagEntry> = emptyList(),
)

@Serializable
public data class OllamaEmbeddingTagEntry(
    val name: String,
...
@Serializable
internal data class OllamaEmbeddingPullRequest(
    val name: String,
...
@Serializable
public data class OllamaEmbeddingPullProgress(
