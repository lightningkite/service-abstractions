...
public suspend inline fun <reified T : Any> Cache.get(key: String): T? {
    return get(key, serializersModule.serializer<T>())
}
...
public suspend inline fun <reified T : Any> Cache.set(key: String, value: T, timeToLive: Duration? = null) {
    return set(key, value, serializersModule.serializer<T>(), timeToLive)
}
...
public suspend inline fun <reified T : Any> Cache.setIfNotExists(
    ...
    return setIfNotExists(key, value, serializersModule.serializer<T>(), timeToLive)
}
...
public suspend inline fun <reified T : Any> Cache.modify(
    ...
    modify(key, serializersModule.serializer<T>(), maxTries, timeToLive, modification)


public inline operator fun <reified T> (() -> Cache).get(key: String) =
    CacheHandle<T>(this, key) { this().serializersModule.serializer<T>() }
