...
internal class DbMapLikeFormat(val serializersModule: SerializersModule) {
...
internal abstract class UnderscoreNamedValueEncoder : TaggedEncoder<String>() {
...
@InternalSerializationApi
@OptIn(ExperimentalSerializationApi::class)
internal abstract class UnderscoreNamedValueDecoder : TaggedDecoder<String>() {
    var lastMapKey: String = ""
    final override fun SerialDescriptor.getTag(index: Int): String =
        when (kind) {
            StructureKind.CLASS -> nested(elementName(this, index))
...
internal class DbLikeMapDecoder(
...
@OptIn(InternalSerializationApi::class)
internal class ListDecodeMapper(val decodeInstance: (index: Int) -> Decoder, val count: Int) : CompositeDecoder {
    var index = 0
    override fun decodeElementIndex(descriptor: SerialDescriptor): Int {
...
@OptIn(InternalSerializationApi::class)
internal class MapDecodeMapper(
...
@OptIn(InternalSerializationApi::class)
internal class DbLikeMapEncoder(
    override val serializersModule: SerializersModule,
    ...
    override fun beginStructure(descriptor: SerialDescriptor): CompositeEncoder {
        return when (descriptor.kind) {
            ...
            is StructureKind.LIST -> {
                val t = currentTag
                val lists =
                    descriptor.getElementDescriptor(0).columnType(serializersModule).map { it.key.joinToString("__") }.associateWith {
                        val l = ArrayList<Any?>()
            ...
            is StructureKind.MAP -> {
                ...
                val valueLists =
                    descriptor.getElementDescriptor(1).columnType(serializersModule).map { it.key.joinToString("__") }.associateWith {
                        val l = ArrayList<Any?>()
