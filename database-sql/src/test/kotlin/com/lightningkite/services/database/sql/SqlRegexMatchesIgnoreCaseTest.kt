 ...
 * `caseSensitive = true` into Exposed's `RegexpOp`, so `ignoreCase` was silently dropped on SQL
 * backends while still honored in-memory and on Mongo — the same query returning different rows
 * depending on backend.
 */
class SqlRegexMatchesIgnoreCaseTest {
    private val database: com.lightningkite.services.database.Database by lazy {
        SqlDatabase("test", TestSettingContext(EmptySerializersModule())) {
            PooledDatabase(Database.connect("jdbc:h2:mem:regexIgnoreCaseTest;DB_CLOSE_DELAY=-1", "org.h2.Driver"), null)
        }
    }
