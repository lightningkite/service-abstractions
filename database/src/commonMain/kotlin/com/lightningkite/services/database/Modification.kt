...
sealed class Modification<T> {
    ...
    infix fun then(other: Modification<T>): Chain<T> = Chain(listOf(this, other))
    ...
    data class ListPerElement<T>(val condition: Condition<T>, val modification: Modification<T>) :
        Modification<List<T>>() {
    ...
    data class SetPerElement<T>(val condition: Condition<T>, val modification: Modification<T>) :
        Modification<Set<T>>() {
    ...
    data class OnField<K, V>(val key: SerializableProperty<K, V>, val modification: Modification<V>) :
        Modification<K>() {
