# Metrics Migration Guide — coroutine-first telemetry API

Migrate services off the old telemetry helpers (`OpenTelemetrySub.span(...)`, `RedMetrics`/`redMetrics`/
`recordOperation`) onto the coroutine-first telemetry API in
`basis/src/commonMain/kotlin/com/lightningkite/services/telemetry/TelemetryBackend.kt`. Redis is the exemplar; this
...
- `telemetryTrace(opName, attributes = ..., dimensions = ...) { span -> ... }`
...
- `span.enrich(TelemetryAttributes { put(...) })` — attach attributes discovered *during* the op (rows
  returned, `cache.hit`, sanitized query). Lands on the span; also readable as a RED dimension if its
  key is listed in `dimensions`.
- `telemetryAttributes(attributes) { ... }` — enrich the ambient bag without opening a span (rare).
- Instruments (only if the service used counters/gauges/in-flight — see below):
  `telemetryHistogram(name, unit, defaultDimensions)`, `telemetryCounter(...)`, `telemetryInFlight(...)`,
  `telemetryGauge(name, unit) { sampleLong }`.
- `TelemetryAttributes { put(TelemetryKey.OfString(name), value) }`, `MetricUnit.{Seconds,Bytes,Occurrences}`.

All of these are no-ops when `context.telemetryBackend` is the default `TelemetryBackend.Noop`, so no null-checks at call sites.
...
private fun spanAttributes(key: String): TelemetryAttributes = TelemetryAttributes {
    ...
    put(TelemetryKey.OfString("cache.key"), TelemetrySanitization.hashCacheKey(key))
}

override suspend fun <T> get(key: String, serializer: KSerializer<T>): T? =
    telemetryTrace("get", attributes = spanAttributes(key), dimensions = setOf(Cache.TelemetryKeys.cacheHit)) { span ->
        val raw = lettuceConnection.get(key).awaitFirstOrNull()
        span.enrich(TelemetryAttributes { put(Cache.TelemetryKeys.cacheHit, raw != null) })
        raw?.let { json.decodeFromString(serializer, it) }
    }
```

Notes on the transform:
- Static `configure {}` attributes become the `attributes =` bag using the `TelemetryAttributes { put(...) }` builder.
- `span?.setAttribute(...)` for result-derived values becomes `span.enrich(TelemetryAttributes { ... })`
...
override suspend fun insert(models): List<Model> = telemetryTrace("insert") { span -> ... }
```

- Drop the `private val metrics = otel.redMetrics(system = ...)` field entirely — `telemetryTrace`
  records RED automatically using `owner.name` as `system`.
- `recordOperation("op", metricAttributes = ...)` → `telemetryTrace("op", attributes = ...)` and/or
  `dimensions = setOf(...)` for the low-card keys that were in `metricAttributes`.
- `configureSpan = { setAttribute(...) }` static attrs → `attributes =`.
- `cacheHit(hit)` helper → `dimensions = setOf(Cache.TelemetryKeys.cacheHit)` + `span.enrich(TelemetryAttributes { put(Cache.TelemetryKeys.cacheHit, hit) })`.
...
- **Remove now-unused imports** of `com.lightningkite.services.otel.*`: `OpenTelemetrySub`,
  ...
  `com.lightningkite.services.telemetry.TelemetryAttributes` and `com.lightningkite.services.telemetry.telemetryTrace`.
- **KEEP driver/SDK instrumentation as-is.** Anything that passes `context.openTelemetry` *into* a
  ...
  driver command spans parent under your `telemetryTrace` span; do not remove it.
...
- Pool/connection gauge → `telemetryGauge(name, MetricUnit.Occurrences) { currentValue }`.
...
- A standalone counter → hold `val c = telemetryCounter(name, unit, defaultDimensions)` in a `val`, call
  `c.increment()`. (Note `increment`/`record` are `suspend`.)
- A histogram for sizes/counts (not durations — durations come free from `telemetryTrace`) →
  `val h = telemetryHistogram(name, MetricUnit.Bytes, defaultDimensions)`, `h.record(amount)`.
- In-flight / concurrency gauge → `val g = telemetryInFlight(name, defaultDimensions)`;
