...
fun KSType.toKotlinSerializer(contextualTypes: List<KSDeclaration>): String {
    ...
    return run {
        ...
        (this.declaration as? KSTypeParameter)?.let {
            return@run it.name.asString().replaceFirstChar { it.lowercase(Locale.getDefault()) }
        }
...
fun KSAnnotated.annotation(name: String, packageName: String = "com.lightningkite.services.data"): KSAnnotation? {
